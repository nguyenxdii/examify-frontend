...
import { 
  ...
  Home, Share2, Eye, Sparkles
} from "lucide-react";
import { getExamDetail, getQuestions, submitPublicQuiz } from "../api/examApi";
...
export default function PublicQuiz() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (examRes.data.status !== "shared") {
          toast.error("Đề thi này không công khai");
          navigate("/");
          return;
        }

        const isShuffled = examRes.data.shuffled;
        ...
        setTimeLeft((examRes.data.duration || 10) * 60);
      } catch (err) {
  ...
  const handleSubmit = async (force = false) => {
    ...
    try {
      const res = await submitPublicQuiz(examId, {
        studentName,
        answers
      });
      setResult(res.data);
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background">
      ...
      <p className="text-muted-foreground font-bold animate-pulse">Đang chuẩn bị đề thi...</p>
    </div>
  );
      ...
      >
        <div className="text-center space-y-5">
          ...
          <p className="text-muted-foreground text-xs max-w-sm mx-auto leading-relaxed">
            {exam?.description || "Chào mừng bạn đến với bài thi trắc nghiệm trực tuyến. Vui lòng điền thông tin để bắt đầu."}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-muted/30 p-5 rounded-2xl border border-border/50 text-center space-y-1">
            ...
            <p className="text-lg font-black">{exam?.duration} phút</p>
          </div>
          <div className="bg-muted/30 p-5 rounded-2xl border border-border/50 text-center space-y-1">
        ...
        <div className="space-y-5">
          ...
          >
            Bắt đầu làm bài <ChevronRight className="w-5 h-5" />
          </button>
        </div>
      </motion.div>
    </div>
          ...
          <aside className="w-full md:w-72 flex flex-col gap-5">
            <div className="bg-card border border-border rounded-[2rem] p-5 shadow-xl space-y-5">
              ...
              <div className="pt-2 border-t border-border flex items-center gap-2 px-1">
                <input 
                  type="checkbox" 
                  id="autoAdvancePublic"
                  checked={autoAdvance}
                  onChange={(e) => setAutoAdvance(e.target.checked)}
                  className="w-4 h-4 rounded border-border text-primary focus:ring-primary"
                />
                <label htmlFor="autoAdvancePublic" className="text-[11px] font-bold text-muted-foreground select-none cursor-pointer">
                  Tự động chuyển câu
                </label>
              </div>
            </div>
        ...
        >
          ...
          <div className="bg-card border border-border rounded-[2.5rem] shadow-2xl p-6 md:p-10 text-center space-y-8 relative overflow-hidden">
            ...
            <div className="space-y-3 relative">
              <h2 className="text-2xl font-black">Kết quả bài làm</h2>
              <p className="text-muted-foreground font-bold uppercase tracking-widest text-[10px]">{studentName}</p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              <button 
                ...
                className="px-6 py-3.5 bg-muted hover:bg-muted/80 text-foreground font-black rounded-xl flex items-center gap-2 transition-all uppercase tracking-wide text-[10px]"
              >
